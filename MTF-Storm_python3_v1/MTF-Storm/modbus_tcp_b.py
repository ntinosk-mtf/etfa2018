                ...
                #return response                                       
                    if fuzz_session.receive_flag==False :
                        ...
                        if fuzz_session.receive_timeout==10 : 
                            ...
                            lgr.info('')
                            lgr.warn('Connection it lost after %d request..it can close/Overflow buffer..do_open RESET!'%fuzz_session.receive_timeout);time.sleep(2.0)
...
class TcpMbap_b(modbus_tcp.TcpMbap):
    ...
    def check_response_b(self, request_mbap, response_pdu_length):
        ...
        if len(error_str)>0:
            lgr.warn('ModbusInvalidMbapError %r.' % (error_str))    
    ...
    def _check_ids_b(self, request_mbap):
        ...
        if (request_mbap.length != fuzz_session.length_not_f) or (request_mbap.length == 0) or (request_mbap.length > 254):
            #lgr.warning('The fuzz_session.length_not_f is : %d' % fuzz_session.length_not_f)
            #lgr.warning('The request_mbap.length is : %d' % request_mbap.length)
...
class TcpQuery_b(modbus_tcp.TcpQuery,modbus.Query):
    ...
    def build_request_b(self,pdu,slave):
        ...
        adu,pdu=p.init_new_session(pdu,slave)  # call the fuzzing mode and fuzzing          
        ...
        elif fuzz_session.priority==1 :                   #fuzz instanse mbap for  response_mbap.check_response_b  
            self._request_mbap.unpack(adu)
            fuzz_session.length_not_f= len(pdu)+1
            return adu+pdu
        else : #case not fuzz testing--testing!!
            fuzz_session.length_not_f= len(pdu)+1
            pass    
