...
import ctypes
import threading
import modbus_tk #Use Modbus TestKit
...
from utils_b import *
import scapy.layers.l2  #Use scapy TestKit
import scapy.layers.inet
from scapy.all import *
from product.message import *   #library from pymodbus v2.1.0   
from product.serial_message import * #TestQueries TestQueriesSerialFC
...
class Master_b(modbus.Master):
    ...
    def execute_master(self,slave,pdu,expected_length=-1):
        ...
        lgr.info('')                                     
        lgr.info('The request pdu: ---> %r '% ByteToHex(pdu))
                ...
                if slave != 0:
                    ...
                    if fuzz_session.priority==3 or fuzz_session.priority==2 or fuzz_session.priority==1 :    
                        ...
                        #extract the pdu part of the response/analyze the received data Response message analyzer"""
                            ...
                            #base chech for log files in the  error code or exception Code out of specifications, 
                            #error code (return code) fc+128, valid  exception Code  = ∈ {01,02,03,04}
                            if return_code >= 128:                   
                                    exception_code = byte_2
                                    if return_code==function_code+128 and (1 <= exception_code<= 4):                                      
                                        lgr.warn("ModbusWarn >> return_code=%d - exception Code=%d" % (return_code, byte_2)) 
                                        return lgr.info('Answer >> fuzzing address: %s, response %r '  % (fuzz_session.starting_address,(return_code, byte_2)))
                                                                     
                                    elif  (return_code==128 or return_code!=function_code+128)  and exception_code>=0:       
                                        lgr.error("ModbusError (not specifications) >> return_code=%d - exception Code=%d" % (return_code, byte_2))
                                        return lgr.warn('Answer >> fuzzing address: %s, response %r '  % (fuzz_session.starting_address,(return_code, byte_2)))

                                    else : 
                                        lgr.error("ModbusError(not specifications)!! >> return_code=%d - exception Code=%d" % (return_code, byte_2))   
                                        return lgr.warn('Answer >> fuzzing address: %s, response %r '  % (fuzz_session.starting_address,(return_code, byte_2))) 
                            #case return code <128
                            elif return_code!= function_code :
                                lgr.critical("ModbusError(not specifications)!! >> return_code=%d - request_function code=%d" % (return_code,function_code ))   
                                lgr.critical('Answer out of spec >> fuzzing address: %s response %r '  % (fuzz_session.starting_address,ByteToHex(response)))
                                return lgr.warn('Answer >> fuzzing address: %s, response:(return_code != function_code) %r '  % (fuzz_session.starting_address,(return_code,function_code )))              
                                
                            elif return_code==function_code and len(response_pdu)==2:
                                lgr.critical('Answer out of spec >> fuzzing address: %s, response %r '  % (fuzz_session.starting_address,ByteToHex(response)))
                                return lgr.warn('Answer >> fuzzing address: %s,response:(fc, Byte) %r '  % (fuzz_session.starting_address,(return_code,response_pdu)))              
                                    
                            else:
                                if function_code == READ_COILS or function_code == READ_DISCRETE_INPUTS:                               
                                    # get the values returned by the reading function
                                    byte_count=byte_2 ;data_format = ">"+(byte_count*"B")
                                    ...
                                    if  (byte_count != len(data) or len(data)==0 or len(data)>250 ):                                   
                                        ...
                                        #byte_count=byte_2                        #set byte count response and calculate
                                        lgr.critical('Answer >> FCs %d, fuzzing address: %s, fuz_quantity_of: %d, response: %r '  % (return_code,fuzz_session.starting_address,fuzz_session.quantity_of_x, ByteToHex(response)))
                                        
                                        #returns the data as a tuple according to the data_format
                                        #(calculated based on the function or user-defined)
                                        data_format = ">"+(len(data)*"B")  # case  byte_2=0 
                                        result=b"";nb_of_digits=len(data)*8                             
                                        if len(data) !=0 :
                                            result = struct.unpack(data_format, data)
                                            if nb_of_digits > 0:
                                                digits = []
                                        ...
                                        return lgr.info('Answer >> fuzzing address: %s, result:  %r '  % (fuzz_session.starting_address,result)) 
                                    
                                elif function_code == READ_INPUT_REGISTERS or function_code ==READ_HOLDING_REGISTERS:

                                    #get the values returned by the reading function
                                    nb_of_digits=0;data = response_pdu[2:]
                                    #response byte count
                                    byte_count = byte_2 ;                                                                                                                                                                  
                                    data_format = ">"+((byte_count//2)*"H") 
                                    ...
                                    if byte_count != len(data) or len(data)==0 or len(data)>250 or  len(data) %2!=0:                  
                                        ...
                                        #byte_count=byte_2 set byte count response and calculate
                                        lgr.critical('Answer >> FCs %d , Fuz_address: %s, fuz_quantity_of: %d, response: %r '  % (return_code,fuzz_session.starting_address,fuzz_session.quantity_of_x, ByteToHex(response)))                                                                                                                     
                                        result=b""
                                        ...
                                        return (lgr.info('Answer >> Fuz_address: %s quantity: %d result:  %r '  % (fuzz_session.starting_address,fuzz_session.quantity_of_x,result)))
                                ...
                                # returns what is returned by the slave after a writing function /return tumple (results)       
                                    ...
                                    if  len(data) ==0 or len(data) < 2 :
                                         return lgr.critical('ModbusInvalidResponseError  >> fuzzing address: %s, return_code: %d, data: %r '  % (fuzz_session.starting_address,return_code, ByteToHex(data)))
                                    #if  return_code  bad
                                    else :
                                        if (return_code == WRITE_MULTIPLE_REGISTERS) or (return_code == WRITE_MULTIPLE_COILS) :
                                            pass
                                        else :
                                            return lgr.critical('ModbusInvalidResponseError/bad code >> fuzzing address: %s, return_code: %d, data: %r '  % (fuzz_session.starting_address,return_code, ByteToHex(data)))
                                        ...
                                        # check out of spec response valid case out boundaries or len case t=3
                                            ...
                                            return lgr.critical('Answer out of spec, len is invalid>> Fuz_address: %s, response: %r '  % (fuzz_session.starting_address,result))
                                     ...
                                     if  len(data) !=0 and len(data) < 4 :         #bad output value
                                        ...
                                        if len(pdu[3:])> 2 and len(pdu[3:]) %2==0: return lgr.warn('ModbusInvalidResponse  >> Fuz_address: %s, send_Output_value (32b):  %r, return_value: %r '  % (fuzz_session.starting_address,ByteToHex(pdu[3:]), ByteToHex(data[2:])))
                                        
                                        if  len(pdu[3:5])!=2 or len(response_pdu[3:])!=2:return lgr.error('ModbusInvalidResponse  >> Fuz_address: %s, send_Output_value:  %r, return_value: %r '  % (fuzz_session.starting_address,ByteToHex(pdu[3:]), ByteToHex(data[2:])))
                                        ...
                                        if (return_code == WRITE_SINGLE_COIL) : 
                                            if fuzz_session.flag_boundaries==1 or fuzz_session.mbap_error==1:
                                               return lgr.critical('Answer out of spec >> Fuz_address: %s, quantity_of_x: %s, output_value: %s'  % (fuzz_session.starting_address,fuzz_session.quantity_of_x,fuzz_session.output_value))
                                                   ...
                                                   return lgr.info('ModbusvalidResponse  (ModbusWriteCoils) >> Fuz_address: %s, send_Output_value: %r, value: %r '  % (fuzz_session.starting_address,ByteToHex(pdu[3:]), ByteToHex(data[2:])))                                                   
                                            else :    
                                                   return lgr.critical('ModbusInvalidResponse  >> Fuz_address: %s, send_Output_value:  %r, Return_value: %r '  % (fuzz_session.starting_address,ByteToHex(pdu[3:]), ByteToHex(data[2:])))
                                            ...
                                            if fuzz_session.flag_boundaries==1 or fuzz_session.mbap_error==1:
                                               return lgr.critical('Answer out of spec >> Fuz_address: %s, quantity_of_x: %s, output_value : %s'  % (fuzz_session.starting_address,fuzz_session.quantity_of_x,fuzz_session.output_value))
                                                   ...
                                                   return lgr.info('ModbusvalidResponse (ModbusWriteRegister) >> Fuz_address: %s, send_Output_value: %r, echo value: %r '  % (fuzz_session.starting_address,ByteToHex(pdu[3:]), ByteToHex(data[2:])))                                                   
                                            else :
                    ...
                    lgr.warn('check response problem')
                    return    
    ...
    def execute_fpdu(self,slave,pdu,expected_length=-1):            
                ...
                if slave != 0:
                    ...
                    # extract the pdu part of the response (mbap+pdu)
                    # extract the pdu part of the response
                    if len(response)==0:                                                        
                       lgr.error('response Modbus message: None')                                                                    
                       return response
    ...
    def dissect(self,pdu,response_pdu) :
                ...
                if len(response_pdu)<2:
                    if  len(response_pdu) ==0 :
                        lgr.critical("ModbusError(not specifications)!! >> return_code and exception Code bad %r" %response_pdu)
                        return b""
                    else:#len(response_pdu) < 2 :                                  
                        (return_code,)=struct.unpack(">B", response_pdu[0:1])
                        return (return_code,)
                      
                else :
                    (return_code,byte_2) = struct.unpack(">BB", response_pdu[0:2])                  #extract to tuple
                ...
                '''
                if return_code== Read_File_record:
                     data = response_pdu[1:] 
                     ...
                     if len(data)>252 or len(data)<7  :                                                
                        ...
                        if len(data)<252 or len(data)>7 :
                            response_length, reference_type = struct.unpack('>BB', data[0:2])
                            if response_length!=len(response_pdu[2:]) or reference_type!=6 : #response_length-byte count ,Sub-Req1-reference_type
                                lgr.error("ModbusError(not specifications)!! >>response_length=%d - reference_type(Sub_Req1)=%d" % (response_length,reference_type))
                                return ByteToHex(data)        #look ref, param data: The packet data to decode (reference_type != 0x06 or reference_type== 0x06)
                        lgr.info('Decode Modbus message response')
                        handle  = ReadFileRecordResponse();message=handle.decode(data)                             
                        
                        if  message==b"" : 
                            lgr.warn('problem decode Modbus message response') ;return ByteToHex(data)       
                        return message 
                ...
                #Decodes the incoming request 
                     ...
                     if len(data)>252 or len(data)<7:                              #bad len response legal PDU=253 bytes
                         ...
                         lgr.info('Echo Modbus message response')
                         return  records                                           
                     else : 
                        if len(data)<252 or len(data)>7 :
                            response_length, reference_type = struct.unpack('>BB', data[0:2])
                            if response_length!=len(response_pdu[2:]) or reference_type!=6 : #response_length-byte count ,Sub-Req1-reference_type
                                ...
                                return ByteToHex(data)        #look ref, param data: The packet data to decode (reference_type != 0x06 or reference_type== 0x06)
                        lgr.info('Decode Modbus message response')
                        handle  = WriteFileRecordResponse() ;message=handle.decode(data)                             
                ...
                #23 (0x17)-The normal response contains the data from the group of registers that were read.
                    ...
                    if (fuzz_session.flag_boundaries==1 or fuzz_session.mbap_error==1 or fuzz_session.quantity_to_Write>121 or fuzz_session.quantity_to_Read>125):
                        ...
                        return lgr.critical('Answer out_of_spec>> test_read_address: %r test_write_address: %r result: %r' % (fuzz_session.read_starting_address,fuzz_session.write_starting_address,result))
                ...
                #fuzz_session.flag_boundaries==1 case invalid address          
                    ...
                    if len(data)!=6:                              
                         ...
                         if (fuzz_session.flag_boundaries==1 or fuzz_session.mbap_error==1 ):                            
                             fuzz_session.flag_boundaries=0 ;lgr.critical('Answer out of spec>> Decode Modbus message response') ;return ByteToHex(data)                                        
                         lgr.info('Decode Modbus message response')                                                #                          
                         return ByteToHex(data)                        
